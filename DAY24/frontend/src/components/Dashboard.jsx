...
const Dashboard = ({ user, onUserUpdate, onLogout }) => {
  ...
  const handleProfilePicUpload = async (e) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      setMessage('Please select an image file (JPEG, PNG, GIF, or WebP)');
      return;
    }
        ...
        <div className="profile-section">
          ...
          <div className="profile-info">
            <div className="profile-picture">
              <img 
                src={profilePic || 'https://via.placeholder.com/150x150/667eea/ffffff?text=' + encodeURIComponent(user.name.charAt(0).toUpperCase())} 
                alt={user.name}
                className="profile-avatar"
                onError={(e) => {
                  console.log('❌ Profile picture failed to load, using fallback');
                  e.target.src = 'https://via.placeholder.com/150x150/667eea/ffffff?text=' + encodeURIComponent(user.name.charAt(0).toUpperCase());
                }}
              />
